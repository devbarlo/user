...
async def article_builder(event, method):
    ...
    title = "cr Userbot"
    description = "Button menu for crUserbot"
    if method == "help":
        help_info = main_menu()
        title = "Help Menu"
    ...
    if method == "ls":
        ...
        title = "SỖƱⱤƇƐ ƇⱤịṤͲịⴂƐ"
        description = "Deploy yourself."
        query = "𝗗𝗲𝗽𝗹𝗼𝘆 𝘆𝗼𝘂𝗿 𝗼𝘄𝗻  ƇⱤịṤͲịⴂƐ bot."
        buttons = [
            (
                Button.url("dev", "@dr_criss"),
                Button.url("dev", "@wjj_u"),
                Button.url("dev", "@@bar_lo0o"),
                Button.url("dev", "@@cr_amr_sabry"),
            )
        ]
