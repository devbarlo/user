...
You have {warns}/{totalwarns} warns until you get blocked by the crtUserbot.
...
@cruser.cru.on(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@cruser.cru.on(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@cruser.cru.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@cruser.cru.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@cruser.cru.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@cruser.cru.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@cruser.cru.on(
    ...
    info={
        ...
        "usage": [
            "{tr}ق/قبول <username/reply reason> in group",
            "{tr}ق/قبول <reason> in pm",
        ],
    },
)
...
@cruser.cru.on(
    pattern="(ر|رفض)(?:\s|$)([\s\S]*)",
    command=("ر", plugin_category),
    info={
        "header": "To disapprove user to direct message you.",
        "note": "This command works only for approved users",
        "options": {"الكل": "To disapprove all approved users"},
        "usage": [
            "{tr}ر/رفض <username/reply> in group",
            "{tr}ر/رفض in pm",
            "{tr}ر/رفض الكل - To disapprove all users.",
        ],
    },
)
...
@cruser.cru.on(
    ...
    info={
        ...
        "usage": [
            "{tr}الغاء حظر <username/reply reason> in group",
            "{tr}الغاء حظر <reason> in pm",
        ],
    },
)
