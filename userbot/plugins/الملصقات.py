...
# modified and developed by @wjj_u , @dr_criss 
...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"cruserbot_{userid}_{pack}_anim"
    if is_video:
        return f"cruserbot_{userid}_{pack}_vid"
    return f"cruserbot_{userid}_{pack}"


def char_is_emoji(character):
    return character in catemoji.UNICODE_EMOJI["ar"]
...
@cruser.cru_cmd(
    pattern="ملصق اختار(?:\s|$)([\s\S]*)",
    command=("اختار ملصق", plugin_category),
    info={
